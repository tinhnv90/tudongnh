@extends('Layout')
@section('title','Sản phẩm ưa thích')
@section('description','Sản phẩm ưa thích')
@section('keywords','Sản phẩm ưa thích')
@section('stylesheet')
...
<div id="content" class="w100min">
	<div id="conten-right">
		<div class="title-page">
			<h3>
				<span>SẢN PHẨM ƯA THÍCH CỦA BẠN</span>
			</h3>
		</div>
		...
		<div class="container w100min cart">
			...
			<ul class="w100min">
				...
				<li class="w100min" data-idproduct="{{$product['idproduct']}}">
					...
					<div class="col-number">
						<p>{{Session::get('productInTheWishlist.'.$product['idproduct'])[0]}}</p>
					</div>
					<div class="col-price">
						<p>@if($product['get_detail']['price']==0)Liên Hệ @else {{$product['get_detail']['price']}} @endif</p>
					</div>
					<div class="col-action">
						<p class="remove-wishlist" 
